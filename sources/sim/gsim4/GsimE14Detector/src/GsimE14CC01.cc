 ...
 *  @file
 *  @brief  GsimE14CC01
 *  $Id: GsimE14CC01.cc,v 1.3 2006/12/06 19:04:45 nanjo Exp $ 
 *  $Log: GsimE14CC01.cc,v $
...
GsimE14CC01::GsimE14CC01(std::string name,
  ...
  : GsimDetector(name,motherDetector,transV,rotV,userFlag)
{
  ...
  //if ( DetUseCC01[0] ){ //////
  //  GSPOS( "CC2H", 1, "CC01", 0,0,0, 0, "ONLY" );
  // }
...
//            p_brass2, sizeof(p_brass2)/sizeof(float),ivol);
    ...
    trapC2S1->setSensitiveDetector("CC01",-1);
    ...
    trapC2S2->setSensitiveDetector("CC01",-1);
...
//     GSPOS("C2A2",icp_al2++,"SEG2",x, y, z, 0, "ONLY");
  ...
  setThisAndDaughterBriefName("CC01");
}


GsimE14CC01::~GsimE14CC01()
{
  ;
}


void GsimE14CC01::setFastSimulationLevel(int level)
{
#ifdef GSIMDEBUG
  ...
  if(level==6) {
    for(std::list<GsimDetector*>::iterator it=m_detList.begin();
	it!=m_detList.end();it++) {
      (*it)->setSensitiveDetector("CC01",0);
    }
  }
